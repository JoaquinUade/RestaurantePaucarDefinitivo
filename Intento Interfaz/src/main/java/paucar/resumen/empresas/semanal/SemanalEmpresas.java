...
public class SemanalEmpresas extends BorderPane {
    ...
    private void cargarVentasSemanalesEmpresas() {
        ...
        if (empresa == null || empresa.isBlank()) {/*Verifica si no se ha seleccionado ninguna empresa o
            ...
            return;/*Si no se ha seleccionado una empresa válida, se detiene la ejecución de este
                    método para evitar cargar datos incorrectos o vacíos en las tablas de ventas
                     y deudas.*/
        }
    ...
    private ComboBox<String> crearComboEmpresas(FilteredList<String> empresasFiltradas) {
        ...
        AtomicBoolean ActualizarTrasAccionUser = new AtomicBoolean(false);/*sirve para que cuando el programa escribe
                                                                                       el texto del ComboBox (por ejemplo al
                                                                                       seleccionar una empresa), no se ejecute el
                                                                                       filtrado como si el usuario hubiera escrito*/
        ...
        cbEmpresa.getEditor().textProperty().addListener((obs, a, TextoNuevo) -> {/*Cada vez que cambia el texto
            ...
            String txt = (TextoNuevo == null ? "" : TextoNuevo.trim().toLowerCase());/*Obtiene el texto nuevo y lo
                                                                                      convierte a minúsculas
                                                                                      y le quita los espacios en
                                                                                      blanco de delante y atras*/
        ...
        cbEmpresa.setCellFactory(list -> {/*en el siguiente bloque de codigo personalizamos como se ve y
            ...
            celda.addEventFilter(MouseEvent.MOUSE_PRESSED, mouseEvent -> {/*
                                                                           * cuando el usuario hace clic con el
                ...
                if (!celda.isEmpty()) {/*si el usuario hizo clic sobre una empresa y no sobre un espacio
                    ...
                    ActualizarTrasAccionUser.set(true);/*Marca que el texto del ComboBox va a ser cambiado por el
                                                                 programa y no por el usuario*/
                    try {
