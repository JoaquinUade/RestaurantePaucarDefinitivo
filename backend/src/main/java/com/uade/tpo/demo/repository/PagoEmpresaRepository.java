...
public interface PagoEmpresaRepository extends JpaRepository<PagoEmpresa, Long> {
     ...
     */
    @Query("SELECT p FROM PagoEmpresa p WHERE p.empresaId = :empresaId AND p.mes = :mes AND p.año = :año")
    List<PagoEmpresa> obtenerPagosPorMesYAño(@Param("empresaId") Long empresaId, 
     ...
     */
    @Query("SELECT COUNT(p) FROM PagoEmpresa p WHERE p.empresaId = :empresaId AND p.tipoPeriodicidad = :tipoPeriodicidad AND p.mes = :mes AND p.año = :año")
    Long contarPagosPorTipoYMes(@Param("empresaId") Long empresaId, 
     ...
     */
    @Query("SELECT p FROM PagoEmpresa p WHERE p.empresaId = :empresaId AND p.tipoPeriodicidad = :tipoPeriodicidad AND p.mes = :mes AND p.año = :año")
    List<PagoEmpresa> obtenerPagosPorTipoYMes(@Param("empresaId") Long empresaId, 
                                               @Param("tipoPeriodicidad") TipoPeriodicidad tipoPeriodicidad, 
                                               @Param("mes") Integer mes, 
                                               @Param("año") Integer año);
